...
if(!require(plyr)){install.packages('plyr')} #plotting
library(plyr)

if(!require(ggplot2)){install.packages('ggplot2')} #plotting
library(ggplot2)

source('https://raw.githubusercontent.com/FredHasselman/toolboxR/master/C-3PR.R')#vioQtile function
...
fun_plot <- function(dat_box=tm, y_lim = c(0.1, 1000)){
...
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(10, 1000)) + ggtitle('Basic query')
...
tm = microbenchmark(fun_disk(query), fun_mem(query), fun_base1(), fun_base2())
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(100, 10000)) + ggtitle('Basic aggregation')
dbGetQuery(db_mem, query)
...
tm = microbenchmark(fun_disk(query), fun_mem(query), fun_base1(), fun_base2())
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(10, 1000)) + ggtitle('Basic aggregation on conditional')
dbGetQuery(db_mem, query)
...
tm = microbenchmark(fun_disk(query), fun_mem(query), fun_base1(), fun_base2())
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(100, 10000)) + ggtitle('Aggregation of temporary variable on conditional')
dbGetQuery(db_mem, query)
...
# {dbExecute(db_disk., statements[1]); 
#   dbExecute(db_disk., statements[2])
#   dbExecute(db_disk., statements[3])}
...
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(100, 10000)) + ggtitle('Create table and add to data base')
...
tm = microbenchmark(fun_disk(query), fun_mem(query), fun_base1(), fun_base2())
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(100, 45000)) + ggtitle('Joining two tables')
...
tm = microbenchmark(fun_disk(query), fun_mem(query), fun_base1(), fun_base2())
boxplot(tm)
fun_plot(dat_box = tm, y_lim = c(100, 10000)) + ggtitle('Joining two tables and aggregation')
dbGetQuery(db_disk, query)